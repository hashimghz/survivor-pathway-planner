...
def main() -> None:
    ...
    # ── Edit-mode detection ───────────────────────────────────────────────────
    # The saved-profiles list (app/components/saved_profiles_list.py) sets
    # `edit_profile_id` in session state when the caseworker clicks Edit on a
    # saved profile. We load the existing profile once, stash it under
    # `_edit_profile`, and use its fields as form defaults. On a fresh
    # new-profile visit neither key exists and we fall through to the blank
    # form.
